...
pay.forte_payment_token, pay.id AS payment_credential_id, sfic.payment_gateway
...
WHERE schedule_payment_date = '" . date('Y-m-d') . "' AND sfic.schedule_status = 0 AND sfic.session = '" . $current_session . "' AND pay.default_credit_card =1 AND is_approval='1'");;
        ...
        try {
            foreach ($student_fees_items as $family_data) {
                ...
                if ($family_data->total_amount > 0) {
                    ...
                    $FamilyAccountCredits = $db->get_results("SELECT `ent`.`id` ,`ent`.`amount`, `ent`.`debit_pay_account_id`, `ent`.`credit_pay_account_id` FROM `ss_payment_account_entries` `ent` WHERE (`ent`.`debit_pay_account_id`='" . $FamilyAccountId . "' OR  `ent`.`credit_pay_account_id`='" . $FamilyAccountId . "') and is_force_payment=0 and debit_pay_account_id <> '" .  $CCAccountId . "'");
                    ...
                    if (!empty($family_data->forte_customer_token) && !empty($family_data->forte_payment_token) && !empty($family_data->total_amount)) {
                        ...
                        if ($totalWalletAmount <= 0) {
                            ...
                            $transactions =  $request_result->data->payment_response;
                            $payment_transactions_code = $request_result->data->transactions_code;
                            $transaction_id = $request_result->data->transactionID;
                            $payment_gateway = $request_result->data->payment_gateway;
                            ...
                            if ((strtoupper($payment_transactions_code) == "A01" || strtoupper($payment_transactions_code) == "100" || strtoupper($payment_transactions_code) == "SUCCEEDED")) {
                                ...
                                $response_code = $payment_transactions_code;
                            ...
                            $student_fees_items = $db->query("update ss_payment_sch_item_cron set schedule_status ='" . $payment_status . "',wallet_amount ='0',cc_amount ='" . $amount . "',reason='" . $trxn_msg . "', payment_unique_id='" . $transactionID . "', payment_response_code='" . $response_code . "', payment_gateway='" . $payment_gateway . "', payment_response='" . json_encode($transactions) . "',  updated_at = '" . date('Y-m-d H:i:s') . "' where schedule_unique_id = '" . $family_data->schedule_unique_id . "' AND id='" . $family_data->id . "' ");
                        ...
                        elseif ($totalWalletAmount < $family_data->total_amount && $totalWalletAmount > 0) {
                            ...
                            if ((strtoupper($payment_transactions_code) == "A01" || strtoupper($payment_transactions_code) == "100" || strtoupper($payment_transactions_code) == "SUCCEEDED")) {
                                ...
                                $db->query("update ss_received_payment_txn_ids set status=1 where request_token = '" . $request_token . "'");
                            } elseif (isset($payment_transactions_code) && !empty(isset($payment_transactions_code))) {
                                $message = str_replace("[AMOUNT]", $full_amount, PAYMENT_FAILED);
                                $response_code = $payment_transactions_code;
                                $transactionID = "";
                                $payment_status = 4;
                            ...
                            $student_fees_items = $db->query("update ss_payment_sch_item_cron set wallet_amount ='" . $WalletAmmount . "',cc_amount ='" . $amount . "' ,schedule_status ='" . $payment_status . "',reason='" . $trxn_msg . "', payment_unique_id='" . $transactionID . "', payment_response_code='" . $response_code . "', payment_gateway='" . $payment_gateway . "', payment_response='" . json_encode($transactions) . "', updated_at = '" . date('Y-m-d H:i:s') . "' where schedule_unique_id = '" . $family_data->schedule_unique_id . "' AND id='" . $family_data->id . "' ");
                            if (!$student_fees_items) {
                                $return_resp = array('code' => "0", 'msg' => 'Family Id : ' . $family_data->family_id . ', Dear ' . $family_data->father_first_name . ' ' . $family_data->father_last_name . ' schedule payment Processed failed because data not insert in database');
                                CreateLog($_REQUEST, json_encode($return_resp));
                            }
