...
if ($payment_start_dateTime < $current_dateTime &&  $payment_end_dateTime > $current_dateTime) {
    ...
    $student_fees_items = $db->get_results("SELECT sfic.id,sfic.schedule_unique_id,sch_item_ids, sfic.schedule_payment_date,sfic.total_amount,sfic.cc_amount,sfic.wallet_amount,sfic.payment_unique_id,sfic.payment_response_code,sfic.payment_response,sfic.created_at, sfic.updated_at,sfic.reason, 
...
pay.forte_payment_token, pay.id AS payment_credential_id,sfic.session, sfic.payment_gateway
    ...
    if (count((array)$student_fees_items) > 0) {
        ...
        WHERE t.user_type_code = 'UT00'  limit 1 ");
            ...
            foreach ($student_fees_items as $family_data) {
                ...
                if (!empty($family_data->updated_at)) {
                    $updated_at = Date('Y-m-d h:i:s', strtotime($family_data->updated_at));
                } else {
                ...
                $full_amount = $currency . $family_data->total_amount;
                ...
                if ($family_data->total_amount > 0) {
                    ...
                    if ($AmountDebitFrom == 'CreditCard' || $AmountDebitFrom == 'CreditCard_Wallet') {
                        ...
                        if (!empty($family_data->payment_unique_id) && strtoupper($family_data->payment_response_code) == "A01" || strtoupper($family_data->payment_response_code) == "100" || strtoupper($family_data->payment_response_code) == "SUCCEEDED") {
                            ...
                            $new_status = '1';
                            $payment_statustxn = '1';
                            if ($AmountDebitFrom == 'CreditCard_Wallet') {
                                //---This Entry For Manage Account Credit /Debit -> Amount Entries From CC TO family Account ---//
                                $AmountTransferedcCCToFamily = $db->query("INSERT INTO `ss_payment_account_entries`(`description`, `amount`, `debit_pay_account_id`, `credit_pay_account_id`, `created_on`) VALUES ('Amount Transferred From Credit Card To Family Virtual Account','" . $family_data->cc_amount . "','" . $CCAccountId . "','" . $FamilyAccountId . "','" . $updated_at . "') ");
                                $AmountTransferedcFamilyToSchool = $db->query("INSERT INTO `ss_payment_account_entries`(`description`, `amount`, `debit_pay_account_id`, `credit_pay_account_id`, `created_on`) VALUES ('Amount Transferred From Family Virtual Account To '" . CENTER_SHORTNAME . "' School Account','" . $family_data->cc_amount . "','" . $FamilyAccountId . "','" . $SchoolAccountId . "','" . $updated_at . "') ");
                            }
                        } else {
                        ...
                        $payment_txns = $db->query("insert into ss_payment_txns set session='" . $family_data->session . "', automatic=1, payment_credentials_id='" . $family_data->payment_credential_id . "', payment_gateway='" . $family_data->payment_gateway . "', payment_status='" . $payment_statustxn . "', payment_unique_id='" . $transactionID . "', payment_response_code='" . $family_data->payment_response_code . "', payment_response='" . $family_data->payment_response . "', is_payment_type='3', amount = '" . $family_data->total_amount . "', payment_date='" . $updated_at . "' ");
                    ...
                    if ($AmountDebitFrom == 'Wallet' || $AmountDebitFrom == 'CreditCard_Wallet') {
                        ...
                        if ($AmountDebitFrom == 'Wallet') {
                            ...
                            $payment_txns = $db->query("INSERT INTO `ss_payment_account_entries`(`description`, `amount`, `debit_pay_account_id`, `credit_pay_account_id`, `created_on`) VALUES ('Transferred - Family account to School account','" . $family_data->wallet_amount . "','" . $FamilyAccountId . "','" . $SchoolAccountId . "','" . date('Y-m-d H:i:s') . "') ");
                            ...
                            if ($PaymentStatus == '1') {
                                ...
                                //---Wallet Amount Entries From Family TO ICK School Account ---//
                                $AmountTransfered = $db->query("INSERT INTO `ss_payment_account_entries`(`description`, `amount`, `debit_pay_account_id`, `credit_pay_account_id`, `created_on`) VALUES ('Transferred - Family account to School account','" . $family_data->wallet_amount . "','" . $FamilyAccountId . "','" . $SchoolAccountId . "','" . date('Y-m-d H:i:s') . "') ");
                                $payment_account_entries_id = $db->insert_id;
                    ...
                    $payment_statustxn = '1';
                    //-----------------Amount Entry IN Transaction Table------------------//
                    $payment_txns = $db->query("insert into ss_payment_txns set session='" . $family_data->session . "', automatic=1, payment_credentials_id='" . $family_data->payment_credential_id . "', payment_gateway='" . $family_data->payment_gateway . "', payment_status='" . $payment_statustxn . "', payment_unique_id='" . $transactionID . "', payment_response_code='" . $family_data->payment_response_code . "', payment_response='" . $family_data->payment_response . "', amount = '" . $family_data->total_amount . "', payment_date='" . $updated_at . "', is_payment_type='5' ");
                    $payment_txns_id = $db->insert_id;
                    $is_payment_type = 5;
                }
                ...
                if ($PaymentStatus == 1) {
                    ...
                    $invoice_update = $db->query("update ss_invoice set receipt_id = '" . $receipt_id . "', receipt_date = '" . $updated_at . "', is_due = '1', receipt_file_path = '" . $fullpathReceipt . "', is_type = 3 where schedule_unique_id = '" . $family_data->schedule_unique_id . "' ");
                    $invoce_info_id = $db->get_var("SELECT invo.id FROM ss_invoice_info invoi INNER JOIN ss_invoice invo ON invoi.invoice_id = invo.id WHERE invo.schedule_unique_id = '" . $family_data->schedule_unique_id . "'");
                    if (!empty($invoce_info_id)) {
                        $db->query("update ss_invoice_info set payment_txn_id='" . $payment_txns_id . "' where invoice_id = '" . $invoce_info_id . "'");
                    }
                }
                ...
                foreach ($schedule_item_student as $val) {
                    if ($AmountDebitFrom == 'CreditCard' || $AmountDebitFrom == 'CreditCard_Wallet' || $family_data->total_amount == '0') {
                        ...
                        $student_fees_transactions = $db->query("insert into ss_student_fees_transactions set student_fees_item_id='" . $val . "', payment_txns_id = '" . $payment_txns_id . "', created_at = '" . $updated_at . "'");
                    }
                    if ($AmountDebitFrom == 'Wallet' || $AmountDebitFrom == 'CreditCard_Wallet') {
                        ...
                        if ($payment_txns_id == 0) {
                            $receipt_id = mt_rand();
                            include "../payment/wallet_pdf.php";
                            $dompdf = new Dompdf();
                            $dompdf->loadHtml($html);
                            // (Optional) Setup the paper size and orientation 
                            $dompdf->setPaper('Executive', 'Potrate');
                            ...
                            file_put_contents($pathR . $filenameR . 'receipt.pdf', $outputR);
                            $invoice_update = $db->query("update ss_invoice set receipt_id = '" . $receipt_id . "', receipt_date = '" . $updated_at . "', is_due = '1', receipt_file_path = '" . $fullpathReceipt . "', is_type = 3, pay_acc_enteries_id='" . $payment_account_entries_id . "' where schedule_unique_id = '" . $family_data->schedule_unique_id . "' ");
                        }
                    }

                    //--- Update student fees item ---//
                    $student_fees_items = $db->query("update ss_student_fees_items set schedule_status = '" . $schedule_status . "', updated_at = '" . $updated_at . "' where id = '" . $val . "' ");
                    //--- insert student fees item status_history ---//
                    $tudent_fees_item__history = $db->query("insert into ss_student_fees_item_status_history set student_fees_items_id='" . $val . "' , current_status=0, new_status='" . $new_status . "', comments='" . $comments . "', created_at='" . $updated_at . "', created_by_user_id='" . $created_user_id . "', session ='" . $family_data->session . "', schedule_payment_date = '" . $family_data->schedule_payment_date . "'");
                ...
                if ($count === count((array)$schedule_item_student)) {
                    ...
                    if ($PaymentStatus == '1') { //---------Success Condition---------------//
                        ...
                        $amount_title = 'Paid Amount:';
                        $date_title = 'Payment Date:';
                        ...
                        $title = 'DECLINED PAYMENT INFO';
                        $amount_title = 'Fees Amount';
                        $date_title = 'Date:';
                    }
                ...
                <tr>
                                <td colspan="2" style="text-align: center;">
                                    <div style="font-size: 18px;margin-top:30px; text-align:left;"><u>' . $title . '</u></div>
                                </td>
                            </tr>   
                            <tr>
                    ...
                    $emailbody_support .= '<tr><td style="width: 25%;" class="color2">' . $amount_title . '</td> 
                        <td style="width: 75%; text-align:left;"> ' . $currency . $family_data->total_amount . '</td></tr>

                        <tr><td style="width: 25%;" class="color2">' . $date_title . '</td>
                    ...
                    $db->query("insert into  ss_payment_sch_item_cron_backup  set schedule_unique_id ='" . $family_data->schedule_unique_id . "',  	family_id ='" . $family_data->family_id . "', sch_item_ids='" . $family_data->sch_item_ids . "', schedule_payment_date='" . $family_data->schedule_payment_date . "', total_amount ='" . $family_data->old_total_amount . "', wallet_amount = '" . $family_data->wallet_amount . "', cc_amount = '" . $family_data->cc_amount . "', schedule_status  = '" . $family_data->schedule_status . "', retry_count = '" . $family_data->retry_count . "', session  = '" . $family_data->session . "', is_approval  = '" . $family_data->is_approval . "', reason = '" . $family_data->reason . "', payment_unique_id = '" . $family_data->payment_unique_id . "', payment_response_code= '" . $family_data->payment_response_code . "', payment_response= '" . $family_data->payment_response . "',  is_cancel=1, created_at='" . date('Y-m-d h:i:s', strtotime($family_data->created_at)) . "', updated_at='" . date('Y-m-d h:i:s', strtotime($updated_at)) . "'");
                    $backup_sch_cron = $db->insert_id;
