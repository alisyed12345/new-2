...
WHERE sfi.session='" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "' AND u.is_deleted = 0 AND u.is_locked=0 AND u.is_active=1 AND pay.default_credit_card =1 AND sfi.schedule_notification = 0 AND sfi.schedule_status = 0 AND schedule_payment_date = '" . date('Y-m-d', strtotime($get_cron_info)) . "'
group by sfi.schedule_unique_id,sfi.schedule_status ORDER BY sfi.original_schedule_payment_date ASC";
    ...
    if (count((array)$remainder) > 0) {
        ...
        foreach ($remainder as $remind) {
            ...
            if (empty($paymentcheck)) {
                ...
                if (!empty($remind->forte_customer_token) && !empty($remind->forte_payment_token)) {
                    $payment_sch_item_cron = $db->query("INSERT INTO `ss_payment_sch_item_cron`(`schedule_unique_id`,`family_id`, `sch_item_ids`, `schedule_payment_date`,`total_amount`,`session`, `is_approval`,`created_at`) VALUES ('" . $remind->schedule_unique_id . "','" . $remind->family_id . "','" . $remind->sch_item_id . "','" . $remind->schedule_payment_date . "','" . $final_amount . "','" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "','1','" . date('Y-m-d H:i:s') . "')");
                    ...
                    if ($payment_last_inserted > 0) {
                       ...
                       /*  $Student_invoice_Id = $db->get_var("SELECT id FROM `ss_invoice` where schedule_unique_id = '" . $remind->schedule_unique_id . "' ");
                        ...
                        $db->query("insert into ss_invoice set  family_id='" . $remind->family_id . "',schedule_unique_id='" . $remind->schedule_unique_id . "', invoice_id='" . $invoce_id . "', invoice_date='" . date('Y-m-d') . "', amount='" . $final_amount . "', is_due='0', status = '1', created_at='" . date('Y-m-d H:i:s') . "', invoice_file_path='" . $fullpath . "', created_by='" .$created_user_id. "'");
        ...
        if (!empty($html_body)) {
            ...
            $image_path = image_binary(SCHOOL_LOGO);
            $html = '<!DOCTYPE html>
    ...
    <table style="width:100%; cellpadding:0px; border:0;" cellpadding="0">
        <tr>
            ...
            <td style="width:40%; text-align:right;padding-top:10px">
                <img src="' . $image_path . '" style="max-width: 150px; max-height: 80px;
                vertical-align:top;">
            </td>
        </tr> 
    </table>
