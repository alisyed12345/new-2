...
<div class="page-header page-header-default">
  <div class="page-header-content">
    <div class="page-title">
      <h4 style="display:inline-block">Scheduled Payment Report</h4>
    </div>
  </div>
  <div class="breadcrumb-line">
    <ul class="breadcrumb">
      <li><a href="<?php echo SITEURL."dashboard" ?>"><i class="icon-home2 position-left"></i> Dashboard</a></li>
      <li class="active">Scheduled Payment Report</li>
    </ul>
  </div>
</div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="col-md-1" style="float: right;">
                <span class="divgdt" style="margin-left: -15px;font-weight: bold;">Total: <span id="grand_total">0</span></span>
...
function exportcsv(){
  ...
  var todate = $('#to_date').val();
  var status = $('#status').val();
  var url = "<?php echo SITEURL ?>report/excel/payment_report_excel"+"?fromdate="+fromdate+"&todate="+todate+"&status="+status;
...
function exportpdf(){
  ...
  var todate = $('#to_date').val();
  var status = $('#status').val();
  var url = "<?php echo SITEURL ?>report/pdf/payment_report_pdf"+"?fromdate="+fromdate+"&todate="+todate+"&status="+status;
  window.location.href = url;
}
...
function fillTable( fromdate = '', todate = '', status = '',getter){ 
  ...
  var table = $('.dataGrid').DataTable({
    ...
    responsive: true,
    ajax: {
            url: "<?php echo SITEURL ?>ajax/ajss-report?action=payment_report",
            data:{
                    fromdate:fromdate,
                    todate:todate,
                    status:status
    ...
    'columns': [
      { 'data': 'parent_name',searchable: true,orderable: true,width:'15%'},
      { 'data': 'child_name',searchable: true,orderable: true,width:'15%'},
      { 'data': 'date',searchable: true,orderable: true,width:'15%'}, 
      { 'data': 'amount',searchable: true,orderable: true, width:'10%'},
      { 'data': 'refund_amount',searchable: true,orderable: true, width:'10%'},
      { 'data': 'status',searchable: true,orderable: true, width:'10%'},
    ],
    "order": [[ 0, "desc" ]],
    "columnDefs": [
      ...
      var currency = '<?php echo $currency ?>';
      document.getElementById("grand_total").innerHTML = currency+total.toFixed(2);
