...
if ($_GET['action'] == 'list_homework') {
	if (check_userrole_by_code('UT05') || check_userrole_by_code('UT01') || in_array("su_homework_list", $_SESSION['login_user_permissions'])) {
		...
		if (check_userrole_by_code('UT01')) {
			...
			(select class_name from ss_classes where session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "' and 
			id = h.class_id) as class_name, g.group_name, h.group_id, DATE_FORMAT(h.created_on,'%m/%d/%Y') AS created_on 
			FROM ss_homework h left outer join ss_groups g on h.group_id = g.id where g.group_name IS NOT NULL and h.student_user_id is  null
			and h.session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "'
			UNION
			SELECT h.id, h.homework_text,  h.homework_attechment, (select class_name from ss_classes 
			where session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "' 
			and id = h.class_id) as class_name, 
			concat(s.first_name,' ',s.last_name) as group_name, h.group_id, DATE_FORMAT(h.created_on,'%m/%d/%Y') AS created_on 
			FROM ss_homework h left outer join ss_student s on h.student_user_id = s.user_id where s.first_name IS NOT NULL  
			and h.session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "'  ORDER BY id DESC", ARRAY_A);

			
			for ($i = 0; $i < count((array)$homeworks); $i++) {
				$homeworks[$i]['created_on']=my_date_changer($homeworks[$i]['created_on']);
...
elseif ($_POST['action'] == 'homework_add') {
	if (in_array("su_homework_create", $_SESSION['login_user_permissions'])) {
		try {
			$describtion = trim($db->escape($_POST['homework_text']));
			$describtion1 = str_replace("<p><br></p>", '', $describtion);
			$describtion1 = str_replace("&nbsp;", '', $describtion);
if (!empty(trim($describtion1))) {
    $db->query('BEGIN');
    $group_id = $_POST['group_id'];
    $class_id = $_POST['subject'];
    $homework_target = $_POST['homework_target'];
    // echo "<pre>";
    // print_r($homework_target);
    // die;
    ...
    //Multiple Group
    $homework_id = [];
    if (count((array)$homework_target) > 0) {
        if (count((array)$homework_target) > 0 && isset($homework_target[0]) && $homework_target[0] == 'whole_group') {
            foreach ($homework_target as $homework_stu_id) {
                $homework = $db->query("insert into ss_homework set session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "', homework_attechment='" . $name . "', homework_text='" . trim($db->escape($_POST['homework_text'])) . "', group_id='" . $group_id . "', class_id='" . $class_id . "', student_user_id = '".$homework_stu_id."', created_by_user_id='" . $_SESSION['icksumm_uat_login_userid'] . "', created_on='" . date('Y-m-d H:i:s') . "', updated_by_user_id='" . $_SESSION['icksumm_uat_login_userid'] . "', updated_on='" . date('Y-m-d H:i:s') . "'");
            ...
            if (count((array)$studentInfo) > 0) {
                foreach ($studentInfo as $info) {
                    $message = "Assalamo alaikum,<br><br>" . CENTER_SHORTNAME . ' ' . SCHOOL_NAME . " homework for your student <strong>" . $info->student_name . " </strong> is given below:<br><br><br> <strong style='text-transform:uppercase'><u>" . $info->class_name . "</u></strong><br><br>" . trim($db->escape($_POST['homework_text'])) . " ";
                    $sql_bulk_msg = $db->query("insert into ss_bulk_message set scheduled_time = '" . date('Y-m-d H:i:s') . "',subject = '" . $subject . "', session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "',message = '" . $db->escape($message) . "', is_report_gen = 0, request_from = 'homework_module', created_on = '" . date('Y-m-d H:i:s') . "', created_by_user_id = '" . $_SESSION['icksumm_uat_login_userid'] . "'");
                    $message_id = $db->insert_id;
                    if ($message_id > 0) {
                        $fullpath_file = $target_file;	//'/'.$target_dir.$name;
                        $db->query("insert into ss_bulk_message_attachment set bulk_message_id = '" . $message_id . "', attachment_file = '" . $name . "' ");
                        if (!empty($info->primary_email)) {
                            $results = $db->query("insert into ss_bulk_message_emails set bulk_message_id = '" . $message_id . "', receiver_email = '" . $info->primary_email . "', is_cc = 0, is_bcc = 0, delivery_status = 2, attempt_counter = 0");
                        }
                        if (!empty($info->secondary_email)) {
                            $results = $db->query("insert into ss_bulk_message_emails set bulk_message_id = '" . $message_id . "', receiver_email = '" . $info->secondary_email . "', is_cc = 0, is_bcc = 0, delivery_status = 2, attempt_counter = 0");
                        }
                    } else {
                        $db->query('ROLLBACK');
            ...
            foreach ($homework_target as $homework_stu_id) {
                ...
                if ($homework_id > 0) {
                    $info = $db->get_row("SELECT DISTINCT sgm.student_user_id, f.primary_email, f.secondary_email, 
												...
												INNER JOIN ss_classes c ON c.`id` = ct.class_id 
												WHERE sgm.latest = 1 
												AND sgm.group_id = '" . $group_id . "' AND ct.class_id = '" . $class_id . "' AND sgm.student_user_id = '" . $homework_stu_id . "' AND u.is_active = 1 AND u.is_deleted = 0 
												AND g.is_active = 1 
												AND g.is_deleted = 0 AND c.is_active = 1 ORDER BY s.`user_id` DESC ");

                    if (!empty($info)) {
                        $message = "Assalamo alaikum,<br><br>" . CENTER_SHORTNAME . ' ' . SCHOOL_NAME . " homework for your kid <strong>" . $info->student_name . "</strong> is given below:<br><br><br> <strong style='text-transform:uppercase'><u>" . $info->class_name . "</u></strong><br><br>" .$_POST['homework_text']. " ";
                                        
                        $sql_bulk_msg = $db->query("insert into ss_bulk_message set scheduled_time = '" . date('Y-m-d H:i:s') . "',subject = '" . $subject . "', message = '" . $db->escape(trim($message)) . "', is_report_gen = 0, session = '" . $_SESSION['icksumm_uat_CURRENT_SESSION'] . "', request_from = 'homework_module', created_on = '" . date('Y-m-d H:i:s') . "', created_by_user_id = '" . $_SESSION['icksumm_uat_login_userid'] . "'");
                        $message_id = $db->insert_id;
                        if ($message_id > 0) {
                            if (!empty($name)) {
                            ...
                            if (!empty($info->primary_email)) {
                                $results = $db->query("insert into ss_bulk_message_emails set bulk_message_id = '" . $message_id . "', receiver_email = '" . $info->primary_email . "', family_id = '" . $info->family_id . "', is_cc = 0, is_bcc = 0, delivery_status = 2, attempt_counter = 0");
                                $message_email_id = $db->insert_id;
                                //$test .= '-primary_email = '.$message_email_id.'---';
                            }
                            if (!empty($info->secondary_email)) {
                                $results = $db->query("insert into ss_bulk_message_emails set bulk_message_id = '" . $message_id . "', receiver_email = '" . $info->secondary_email . "', family_id = '" . $info->family_id . "', is_cc = 0, is_bcc = 0, delivery_status = 2, attempt_counter = 0");
                                $message_email_id = $db->insert_id;
                                //$test .= '-secondary_email = '.$message_email_id.'---';
                            }
                        } else {
                            $db->query('ROLLBACK');
                            if (isset($_POST['submit_form']) && !empty($_POST['submit_form'])) {
				...
				if ($homework_id > 0 && $message_id > 0 && (!empty($info) || count((array)$studentInfo) > 0) && $db->query('COMMIT') !== false) {
					if (in_array($imageFileType, $extensions_arr)) {
						move_uploaded_file($_FILES['homework_attechment']['tmp_name'], $target_file);
					}

					if (isset($_POST['submit_form']) && !empty($_POST['submit_form'])) {
						echo json_encode(array('code' => "1", 'msg' => 'Homework added sucessfully..'));
						exit;
					} else {
						echo json_encode(array('code' => "1", 'msg' => 'Homework added sucessfully..'));
						exit;
					...
					$db->query('ROLLBACK');
					if (isset($_POST['submit_form']) && !empty($_POST['submit_form'])) {
						echo json_encode(array('code' => "0", 'msg' => 'Homework not added'));
						exit;
					} else {
						echo json_encode(array('code' => "0", 'msg' => 'Homework not added'));
						exit;
				...
				if (isset($_POST['submit_form']) && !empty($_POST['submit_form'])) {
					echo json_encode(array('code' => "0", 'msg' => 'Homework cannot be empty'));
					exit;
				} else {
					echo json_encode(array('code' => "0", 'msg' => 'Homework cannot be empty'));
					exit;
