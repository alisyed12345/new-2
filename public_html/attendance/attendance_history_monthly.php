...
if(check_userrole_by_code('UT01')){	
    ...
    WHERE ct.is_active <> 2 AND c.is_active = 1 AND c.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."' 
    AND ct.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."'");
}elseif(check_userrole_by_code('UT02')){ 
    $classes = $db->get_results("SELECT DISTINCT c.id,c.class_name FROM ss_classtime ct INNER JOIN ss_classes c ON ct.class_id = c.id 
    WHERE ct.is_active <> 2 AND c.is_active = 1 AND ct.id in (select classtime_id from ss_staffclasstimemap where staff_user_id = '".$_SESSION['icksumm_uat_login_userid']."' AND active = 1) AND c.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."' AND ct.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."' ");
}elseif(check_userrole_by_code('UT05')){
    $classes = $db->get_results("SELECT DISTINCT c.id,c.class_name FROM ss_classtime ct INNER JOIN ss_classes c ON ct.class_id = c.id 
    WHERE ct.is_active <> 2 AND c.is_active = 1 AND c.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."' AND ct.session = '".$_SESSION['icksumm_uat_CURRENT_SESSION']."'");
} 
?>
...
<div class="page-header page-header-default">
  <div class="page-header-content">
    <div class="page-title">
      <h4 style="display:inline-block">Class Wise Attendance History</h4>
    </div>
  </div>
  <div class="breadcrumb-line">
    <ul class="breadcrumb">
      <li><a href="<?php echo SITEURL."dashboard.php" ?>"><i class="icon-home2 position-left"></i> Dashboard</a></li>
      <li class="active">Class Wise Attendance History</li>
    </ul>
  </div>
</div>
<!-- /page header --> 
            ...
            <div class="col-md-4 showmonthdiv" style="float: right; display:none;">
              <div class="form-check-inline">
                <label class="form-check-label" for="showfullmonth">
                  <input type="checkbox" class="form-check-input" id="showfullmonth" name="showfullmonth"> Show Full Month
                </label>
              </div>
            </div>
          </div>
          <br>
            <div class="row">
              ...
              <div class="col-md-3 col-xs-6"><div class="form-group">
                <input type="hidden" name="action" value="get_attendance_of_class_and_month_based">
                <button type="submit" class="btn btn-success"><i class="icon-spinner2 spinner hide marR10 insidebtn" id="get_spinner"></i> Get Attendance</button>
                </div>
              </div>
            </div>
          </form>
          ...
          <form id="frmAttendanceSheet" class="form-validate-jquery" method="post">
            ...
            <div class="row">
              ...
              <div class="col-md-2 text-right">
                ...
                <input type="hidden" name="action" value="save_attendance_class_based">
